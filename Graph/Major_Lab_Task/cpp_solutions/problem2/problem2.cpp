 ...
 * 
 * Objective: Find minimum cost path using car and metro
...
void printPathInfoWithCost(const Graph& graph, const std::vector<int>& path, 
    ...
    for (size_t i = 0; i + 1 < path.size(); i++) {
        ...
        if (node1 && node2) {
            ...
            std::string modeDisplay = (mode == "metro") ? "Metro" : "Car";
...
int main() {
    // Create graph and load data (car and metro only)
    Graph graph;
    CSVParser parser(graph);
    
    std::cout << "Loading transportation network data..." << std::endl;
    parser.parseRoadmap("../Route-de-Dhaka-master/Dataset/Roadmap-Dhaka.csv");
    parser.parseMetroRoute("../Route-de-Dhaka-master/Dataset/Routemap-DhakaMetroRail.csv");
    ...
    while (true) {
        ...
        printPathInfoWithCost(graph, result.path, source, dest, sameSource, sameDest);
        ...
        generateKML(pathCoords, 2, "problem_2_route.kml");
